...
saveChangesBtn.addEventListener('click', async () => {
    showNotification('⏳ Подготовка к сохранению...');
    
    const dataStr = JSON.stringify(allData, null, 2);
    
    try {
        // Создаём ссылку на создание Issue с предзаполненными данными
        const encodedBody = encodeURIComponent(dataStr);
        const issueUrl = `https://github.com/Alexandr-komi/corporate-directory/issues/new?title=${encodeURIComponent('[SAVE] Обновление справочника сотрудников')}&body=${encodedBody}&labels=data-update`;
        
        // Открываем в новой вкладке
        window.open(issueUrl, '_blank');
        
        showNotification('📝 Открылась страница создания Issue. Нажмите "Submit new issue" для сохранения');
